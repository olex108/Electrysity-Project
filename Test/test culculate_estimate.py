...
# project_parameters = {
# "Приборы (стиральная машина)": "Да",
# "Приборы (посудомоечная машина)": "Да",
# "Приборы (бойлер)": "нет",
# "Приборы (электрическая плита)": "нет",
# "Приборы (водонагреватель)": "нет",
# "Приборы (Духовой шкаф)": "Да",
# "Приборы (дополнительно)": 3
# }

project_parameters = {
"Название": "Remeslennaya 15",
'Тип проекта': 'квартира',
'Площадь помещения': '45',
'Количество комнат': '5',
'Материал стен': 'Бетон',
'Материал перегородок': 'Бетон',
'Потолки Штукатурно-малярные(Да)/подвесные(Нет)': 'нет',
'Количество розеток': '29',
'Количество выключателей': '7',
'Количество розеток слаботочных': '4',
'Количество точек освещения': '8',
'Щит': 'Встроенный',
'Нужна замена вводного кабеля в квартиру': 'нет',
'Приборы (стиральная машина)': 'Да',
'Приборы (посудомоечная машина)': 'Да',
'Приборы (бойлер)': 'Да',
'Приборы (электрическая плита)': 'Да',
'Приборы (водонагреватель)': 'нет',
'Приборы (Духовой шкаф)': 'Да',
'Приборы (дополнительно)': '1',
}
...
def calculate_estimate(project_parameters):
    ...
    project_estimate["Объект:"] = (
        f"{project_parameters["Тип проекта"]}, {project_parameters["Площадь помещения"]}м2"
    )

    # Расчет количества отверстий под подрозетники:
    socket_key = (
        f"Устройство отверстия под подрозетник ({project_parameters["Материал стен"].lower()})"
    )
    socket_count = (
    ...
    if (
        project_parameters["Материал стен"].lower()
        == project_parameters["Материал перегородок"].lower()
    ):
        project_estimate[socket_key] = (
            f"{socket_count}шт * {parameters.price_list[socket_key]} = {socket_price}руб."
        )
    else:
        project_estimate[socket_key] = (
    ...
    if int(project_parameters["Площадь помещения"]) > 140:
        groove_coefficient *= 1.2
    if (
        project_parameters["Тип проекта"] == "дом"
        or project_parameters["Тип проекта"] == "коммерция"
    ):
        groove_coefficient *= 1.2
    ...
    if project_parameters["Материал стен"].lower() != "гипсокартон":
        ...
        if (
            ...
            if project_parameters["Материал перегородок"].lower() != "Гипсокартон" and project_parameters["Материал стен"].lower() != project_parameters["Материал перегородок"].lower():
                groove_key = (
                    f"Штробление стен 20*20 ({project_parameters["Материал перегородок"].lower()})"
                )
                project_estimate[groove_key] = (
    ...
    if (
        int(project_parameters["Количество розеток"])
        / int(project_parameters["Площадь помещения"])
    ) > 1.4:
    ...
    if (
        int(project_parameters["Количество точек освещения"])
        / int(project_parameters["Количество комнат"])
    ) > 2:
    ...
    if (
        ...
        cable_for_light_coefficient = cable_coefficient * 1.4
    cable_count = (
        int(project_parameters["Количество комнат"]) * 20 * cable_for_light_coefficient
    )
    ...
    if project_parameters["Тип проекта"] == "частичная (комната/кухня)":
        ...
        project_estimate[switch_box_key] = (
            f"1шт * {parameters.price_list[switch_box_key]} = "
            f"{parameters.price_list[switch_box_key]} руб."
        )
...
print(calculate_estimate(project_parameters))